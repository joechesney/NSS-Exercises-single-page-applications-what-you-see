...
for (let i = 0; i < celebs.length; i++) {
    ...
    newContent += `<bio>${celebs[i].bio}</bio><br><img src="${celebs[i].image}"/>`;
...
const containerDivElements = document.getElementsByTagName("*");
for (var j = 0; j < containerDivElements.length; j++) {
    containerDivElements[j].addEventListener("click", function (event) {
        for (var y = 0; y < containerDivElements.length; y++){
            containerDivElements[y].setAttribute("style", "border: 0;");
            console.log("border removed");
        }
        containerDivElements[j].setAttribute("style", " border: 3px dotted lightblue;");
        textBox.focus();
